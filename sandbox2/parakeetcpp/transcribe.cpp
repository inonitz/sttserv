...
int main(int argc, char* argv[]) 
{
    ...
    status = parakeet_init_context(commandLineArguments, g_ctx.m_ctxParams, &g_ctx.mh_llmContext);
    if (!status) {
    ...
    g_ctx.m_llmFullParams = parakeet_full_default_params(parakeet_sampling_strategy::PARAKEET_SAMPLING_GREEDY);
    g_ctx.m_llmFullParams.n_threads   = commandLineArguments.m_numThreads;
    g_ctx.m_llmFullParams.offset_ms   = 0;
    g_ctx.m_llmFullParams.chunk_length_ms     = 5000;
    g_ctx.m_llmFullParams.left_context_ms     = 3000;
    g_ctx.m_llmFullParams.right_context_ms    = 5000;
...
cleanup:
    ...
    parakeet_destroy_context(g_ctx.mh_llmContext);
    ...
    while(!g_ctx.m_exit) 
    {
...

        {
            ...
            if(g_ctx.m_recordTimerFlag == false) {
                ...
                g_ctx.m_llmFullParams.duration_ms = __scast(i32, (elapsedTimeNs+1000000-1) / 1000000);
            }


            g_ctx.mr_Start_Till_EndOfInfer.tick();
            success = parakeet_full(
                g_ctx.mh_llmContext, 
            ...
            if(success != 0) {
                ...
                const int n_segments = parakeet_full_n_segments(g_ctx.mh_llmContext);
                for (int i = 0; i < n_segments; ++i) {
                    const char* text = parakeet_full_get_segment_text(g_ctx.mh_llmContext, i);
