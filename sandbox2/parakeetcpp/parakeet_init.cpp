...
bool parse_commandline_args(int argc, char** argv, CommandLineArguments& outParams) {
    ...
    outParams.mk_numSamplesStep      = __scast( i32, (1e-3f*__scast(f32, outParams.m_stepMillisecond   )) ) * __scast(i32, CommandLineArguments::kInferenceSampleRate);
    ...
    outParams.mk_numSamplesThirtySec = __scast( i32, (1e-3f*__scast(f32, 30000.0                       )) ) * __scast(i32, CommandLineArguments::kInferenceSampleRate);
...
bool parakeet_init_context(
    const CommandLineArguments&  inArgParsed,
    ParakeetContextParameters& outCtxParams,
    ParakeetContextHandle*     outContext
) {
    ggml_backend_load_all();
    outCtxParams = parakeet_context_default_params();
    outCtxParams.use_gpu    = inArgParsed.mb_useGPU;
    outCtxParams.gpu_device = inArgParsed.m_deviceID;

    *outContext = parakeet_init_from_file_with_params(
...
void parakeet_destroy_context(
    ParakeetContextHandle outContext
) {
    parakeet_free(outContext);
    return;
}
