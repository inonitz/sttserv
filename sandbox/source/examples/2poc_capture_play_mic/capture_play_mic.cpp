...
int main(int argc, char* argv[]) 
{
    ...
    ma_uint32        cpDeviceIdx  = 0;
    ma_uint32        pbDeviceIdx = 0;
...
void audioCaptureCallbackProducer(
    ...
    while (framesRemaining > 0) {
        ...
        ma_pcm_rb_acquire_write(&userCtx->m_ringBuffer, &framesToWrite, &pWriteBuffer);
        if (framesToWrite > 0) {
            std::memcpy(pWriteBuffer, pInputBytePtr, framesToWrite * bytesPerFrame);
            ma_pcm_rb_commit_write(&userCtx->m_ringBuffer, framesToWrite);

            { /* Lock guard will go out of scope and release the mutex */
                std::lock_guard<std::mutex> lock(userCtx->m_signalMtx);
                userCtx->m_hasData = true;
            }
            userCtx->m_signalCV.notify_one();
